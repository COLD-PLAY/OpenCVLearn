...
# 定义缩放倍数和平移向量把五角星华仔左半部分画面的上方
pentagram = np.round(pentagram * 80 + np.array([160, 120])).astype(np.int)

# print pentagram

# 将5 个顶点作为多边形顶点连线，得到五角星
cv2.polylines(canvas, pentagram, True, (0, 255, 255), 2)
...
for x in range(302, 600):
	...
	# line_color = [int(c) for c in color_pixel[0][0]]
...
# 在左半部分最上方打印文字
cv2.putText(canvas, 'Python-OpenCV Drawing Example', (5, 15), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

cv2.imwrite('output/draw_basic.jpg', canvas)
